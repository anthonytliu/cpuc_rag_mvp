...
DB_DIR = PROJECT_ROOT / "data" / "vector_stores" / "local_lance_db"
...
def load_proceeding_titles():
    ...
    try:
        import json
        # Check multiple possible locations for the proceeding titles file
        possible_paths = [
            PROJECT_ROOT / "proceeding_titles.json",  # Legacy location
            PROJECT_ROOT / "data" / "cache" / "proceeding_titles.json",  # New location
        ]
        
        for titles_file in possible_paths:
...
def get_proceeding_file_paths(proceeding_id: str, base_dir: Path = None) -> dict:
    ...
    # New cpuc_proceedings structure in data/proceedings/
    cpuc_proceedings_dir = base_dir / "data" / "proceedings" / "cpuc_proceedings"
    ...
    return {
        ...
        # Centralized vector DB (in data/vector_stores for shared access)
        'vector_db': base_dir / "data" / "vector_stores" / "local_lance_db" / proceeding_id,
        'document_hashes': base_dir / "data" / "vector_stores" / "local_lance_db" / proceeding_id / "document_hashes.json",
